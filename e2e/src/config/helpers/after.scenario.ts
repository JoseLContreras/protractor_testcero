...
let directorioActualEjecucion = require (path.resolve(process.cwd(), './e2e/src/data/Report.json'));
...
defineSupportCode(({After}) => {
    ...
    async function saveFailedScenarioScreenshot(world: World, testCase: TestCase) {
        ...
        saveScreenshot1(screenshot, fileName);
     ...
     * Save a screenshot
     */
    function saveScreenshot1(screenshot: string, fileName: string) {
        let screenshotAUX = directorioActualEjecucion.GlobalFormato
        const screenshotPath1 = path.resolve(screenshotAUX+'/screenshots/');       
        console.log('pase por after y esto trae la ruta de la captura donde falla'+directorioActualEjecucion.GlobalFormato);

        let filepath1 = path.resolve(screenshotPath1, fileName);
        let stream: WriteStream;
        ensureDirSync(screenshotPath1);
        stream = createWriteStream(filepath1);
